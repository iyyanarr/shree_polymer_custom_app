...
def create_stock_entry(self):
    try:
        # Query to fetch consolidated item details
        ...
        # Determine the source and target warehouses from the first item
        ...
        frappe.logger().debug(f"Warehouses configured: From {source_warehouse}, To {target_warehouse}")

        # Helper function to create the Serial and Batch Bundle doc
        def create_bundle_doc(item_details):
            # Create a new bundle only if batch_no is available
            # or if you need to track your own batch references
            if item_details.batch_no:
                bundle_doc = frappe.get_doc({
                    "doctype": "Serial and Batch Bundle",
                    "item_code": item_details.item,
                    "warehouse": item_details.source_warehouse_id,
                    "type_of_transaction": "Outward",
                    "voucher_type": "Stock Entry",
                    "entries": [{
                        "batch_no": item_details.batch_no,
                        "qty": flt(item_details.qty, 3)
                    }]
                })
        ...
        # Add items to the Stock Entry using the new bundle approach
        for each__item in ddei__items:
            # Attempt to create a corresponding Serial and Batch Bundle
            sab_bundle = create_bundle_doc(each__item)
            ...
            stock_entry.append("items", {
                ...
                "uom": "Kg",
                # assign the new bundle if created
                "serial_and_batch_bundle": sab_bundle if sab_bundle else None,
        ...
        # Update reference in the original doc
...
def make_delivery_note_entry(self):
    try:
        ...
        if customer:
            ...
            for each__item in ddei__items:
                # Add items (if using new bundle system, tie them in similarly)
                spp_dc.append("items", {
                    "scan_barcode": each__item.lot_number,
                    "item_code": each__item.item,
                    "item_name": frappe.db.get_value("Item", each__item.item, "item_name"),
                    # "batch_no": each__item.batch_no,  # Omit if you're using new bundles
                    "qty": flt(each__item.qty, 3),
                    "uom": "Kg",
                    "target_warehouse": each__item.warehouse_id,
            ...
            frappe.throw("Customer not found for the warehouse <b>" + target_warehouse + "</b>")

    except Exception as e:
        frappe.db.rollback()
        rollback__entries(self)
        frappe.log_error(message=frappe.get_traceback(),
                         title="shree_polymer_custom_app.shree_polymer_custom_app.doctype.deflahing_despatch_entry.deflahing_despatch_entry.make_delivery_note")
        self.reload()
...
def rollback__entries(self):
    try:
        ...
        if self.stock_entry_reference:
            ...
            for st__ref in ref:
                # Cleanup Delivery Notes or Stock Entries
...
def make_stock_entry(self):
    try:
        ...
        # Identify process name to change naming series
        ...
        for each in self.items:
            # If using the new bundle system, remove "batch_no" references here as well
            stock_entry.append("items", {
                "item_code": each.item,
                ...
                "qty": flt(each.qty, 3),
                # "batch_no": each.batch_no,       # Remove or adapt for new bundles
                # "spp_batch_number": each.spp_batch_no, # Remove or adapt for new bundles
        ...
        if self.stock_entry_reference:
            ...
            frappe.db.sql(f"""DELETE FROM `tabStock Ledger Entry` 
                              WHERE voucher_type='Stock Entry' AND voucher_no='{self.stock_entry_reference}'""")
            dde = frappe.get_doc(self.doctype, self.name)
            dde.db_set("docstatus", 0)
            dde.db_set("stock_entry_reference", '')
            frappe.db.commit()
        ...
        frappe.log_error(
            ...
            title="shree_polymer_custom_app.shree_polymer_custom_app.doctype.deflashing_despatch_entry.deflashing_despatch_entry.make_stock_entry"
        )
...
def check_available_stock(warehouse, item, batch_no):
    try:
        if batch_no:
            query = f"""SELECT qty FROM `tabItem Batch Stock Balance`
                        WHERE item_code='{item}' AND warehouse='{warehouse}' AND batch_no='{batch_no}'"""
        else:
            query = f"""SELECT qty FROM `tabItem Batch Stock Balance`
        ...
        if qty:
            if qty[0].qty:
                ...
                return {
                    "status": "failed",
                    "message": f"Stock is not available for the item <b>{item}</b>"
                }
        else:
            return {
                "status": "failed",
        ...
        frappe.log_error(
            message=frappe.get_traceback(),
            title="shree_polymer_custom_app.shree_polymer_custom_app.doctype.deflashing_despatch_entry.deflashing_despatch_entry.check_available_stock"
        )
...
def validate_lot_barcode(bar_code):
    try:
        if not frappe.db.get_value("Deflashing Despatch Entry Item", {"lot_number": bar_code, "docstatus": 1}):
            ...
            if not sublot__resp:
                ...
                if ins__prod_resp and ins__prod_resp.get('status') == 'success':
                    ...
                    card_details = frappe.db.get_value(
                        "Job Card", job_card, ["production_item", "total_completed_qty", "work_order"], as_dict=1
                    )

                    query = f"""SELECT
                                    SED.t_warehouse as from_warehouse,
                                    SED.valuation_rate,
                                    SED.amount,
                                ...
                                WHERE SED.item_code='{card_details.get("production_item")}'
                                    AND SE.work_order='{card_details.get("work_order")}' """

                    spp_and_batch = frappe.db.sql(query, as_dict=1)
                    if spp_and_batch:
                        stock_status = check_available_stock(
                            spp_and_batch[0].get("from_warehouse"),
                            card_details.get("production_item"),
                            spp_and_batch[0].get("batch_no", "")
                        )
                    ...
                    frappe.response.message = (
                        f"The <b>Deflashing Despatch Entry</b> for the lot - <b>{bar_code}</b> is already exists..!"
                    )
        else:
            frappe.response.status = "failed"
        ...
        frappe.log_error(
            message=frappe.get_traceback(),
            title="shree_polymer_custom_app.shree_polymer_custom_app.doctype.deflashing_despatch_entry.deflashing_despatch_entry.validate_lot_barcode"
        )
...
def check_sublot(bar_code):
    ...
    if lot_info.get("status") == "success":
        if lot_info.get('data'):
            # This is not covered in workflow, for dispatch to another warehouse
            if lot_info.get('data').get('stock_entry_type') == "Material Receipt":
                return_response(lot_info)
            else:
                # First validate material receipt entry
...
def validate_warehouse(bar_code):
    try:
        ...
        if spp_settings.deflashing_vendor:
            ...
            exe_warehouse = frappe.db.sql(f"""
                SELECT warehouse_name, name, is_group 
                FROM `tabWarehouse`
                WHERE barcode_text='{bar_code}' 
                    AND disabled=0 
                    AND parent_warehouse IN ({condition})
            """, as_dict=1)
        ...
        frappe.log_error(
            message=frappe.get_traceback(),
            title="shree_polymer_custom_app.shree_polymer_custom_app.doctype.deflashing_despatch_entry.deflashing_despatch_entry.validate_warehouse"
        )
