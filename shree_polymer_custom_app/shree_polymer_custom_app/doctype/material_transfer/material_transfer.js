...
frappe.ui.form.on('Material Transfer', {
	...
	view_stock_entry:(frm) =>{
		...
		if(!frm.doc.transfer_date){
			frm.set_value('transfer_date',frappe.datetime.now_date())
			refresh_field('transfer_date')
		}
	},
	...
	refresh: function(frm) {
	    ...
	    if (has_common(frappe.user_roles, ['Mill Operator']) && frappe.session.user!="Administrator") {
			...
			frm.set_value("material_transfer_type","Transfer Compound to Sheeting Warehouse")
	    }
	    if(frm.doc.material_transfer_type=="Transfer Compound to Sheeting Warehouse"){
