...
frappe.ui.form.on('Lot Inspection Entry', {
	refresh: (frm) => {
	...
	"scan_inspector": function(frm) {
		if(frm.doc.scan_inspector && frm.doc.scan_inspector != undefined){
			frappe.call({
				method:'shree_polymer_custom_app.shree_polymer_custom_app.doctype.lot_inspection_entry.lot_inspection_entry.validate_inspector_barcode',
				args:{
	...
	"scan_production_lot": (frm) => {
		if (frm.doc.scan_production_lot && frm.doc.scan_production_lot != undefined) {
			frappe.call({
				method: 'shree_polymer_custom_app.shree_polymer_custom_app.doctype.lot_inspection_entry.lot_inspection_entry.validate_lot_number',
				args: {
				...
				callback: function (r) {
					if (r.message.status == "Failed") {
						...
						frm.set_value("inspected_qty", r.message.message.total_completed_qty)
	...
	"add": (frm) => {
		...
		if (!frm.doc.lot_no || frm.doc.lot_no == undefined) {
			frappe.msgprint("Please Scan the Lot.");
			return
		}
		...
		if (!frm.doc.inspected_qty || frm.doc.inspected_qty == undefined) {
			frappe.msgprint("inspected qty can't be empty.");
			return
		}
		else {
			if ((frm.doc.inspected_qty ? frm.doc.inspected_qty : 0) >= ((frm.doc.rejected_qty ? frm.doc.rejected_qty : 0) + (frm.doc.total_rejected_qty ? frm.doc.total_rejected_qty : 0))) {
				var row = frappe.model.add_child(frm.doc, "Lot Inspection Entry Item", "items");
