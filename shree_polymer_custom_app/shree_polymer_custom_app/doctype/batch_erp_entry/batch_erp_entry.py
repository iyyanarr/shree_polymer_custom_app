...
from shree_polymer_custom_app.shree_polymer_custom_app.api import validate_stock_entry


class BatchERPEntry(Document):
	# def on_trash(self):
	# 	if self.stock_entry_reference:
	# 		st_ids = self.stock_entry_reference.split(',')
	# 		validate_stock_entry(st_ids)
			
	def validate(self):
		if getdate(self.posted_date) > getdate():
			frappe.throw("The <b>Posted Date</b> can't be greater than <b>Today Date</b>..!")
