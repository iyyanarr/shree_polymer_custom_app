...
class Importer:
	...
	def import_data(self):
		...
		# frappe.log_error(message = payloads, title = '--payloads--all files --')
...
class ImportFile:
	def __init__(self, doctype, file, template_options=None, import_type=None):
		...
		# frappe.log_error(title = "-- readed content from sheet --", message = self.raw_data)
	...
	def parse_data_from_template(self):
		...
		for i, row in enumerate(self.raw_data):
			# frappe.log_error(title = "--each row --", message = row)
			...
			if not header:
				...
				# if row[14]!='Y':
				exe__rec = frappe.db.get_all("Imported Batches",filters={"compound_code":row[0],"mixbatchno":row[1],"mix_barcode":row[2],"material":row[4]},fields=['data_import'])
				if not exe__rec:
					...
					for exe__r in exe__rec:
						already_imported_batches.append({"compound_code":row[0],"mixbatchno":row[1],"mix_barcode":row[2],"material":row[4],"batch_erp_id":exe__r.data_import})
		# frappe.log_error(title = "--p_doc parsed data list --", message = parsed_data)
		self.header = header
		self.columns = self.header.columns
		self.data = data
		# frappe.log_error(data,"imported")
		self.check_exeisting_data(already_imported_batches)
		if len(data) < 1:
			frappe.throw(
	...
	def check_exeisting_data(self,data):
		if data:
			url = f"""{frappe.utils.get_url()}/app/error-log/{frappe.log_error(title = "--Already Imported Batches --", message = data).name}"""
			frappe.msgprint(
				_(f""" Import template should contain a Header and atleast one row or data already is imported.Please check the sheet in <b>Batch ERP Entry - {data[0].get('batch_erp_id')}</b> or <a style="color:blue;" href='{url}'>click to view the logs</a>"""),
				title=_("Template Error"),)
...
def create_wos(batch_entry_id):
	...
	if logs and valid_stock:
		...
		# frappe.log_error(title="--imp batch logs ,items groups --group by com,mix -- ",message = items)
		...
		# frappe.log_error(title="-- all ----imp batch logs ,imported_batches",message = imported_batches)
		if items:
			spp_settings = frappe.get_single("SPP Settings")
			try:
				for item in items:
					if frappe.db.get_all("Item",filters={"name":item.get("compound_code")}):
						# frappe.log_error(title="--item entering -- ",message = item.get("compound_code"))
						bom = frappe.db.get_value("Item",item.compound_code,"default_bom")
						if bom:
							# frappe.log_error(title="-- bom entring -- ",message = bom)
							...
							# frappe.log_error(title="-- work order after save-- ",message = wo.as_dict())
				...
				frappe.log_error(message=frappe.get_traceback(),title=f"Work Order Data import failed - {batch_entry_id}")
...
def make_stock_entry(batch_entry_id,spp_settings,work_order_id,compound_code, purpose,mix_barcode,mix_batchno, qty=None):
	...
	try:
		...
		for x in work_order.required_items:
			# frappe.log_error(title="--work order items--", message = x.as_dict())
			t_qty_list = frappe.db.sql(""" SELECT SUM(qty) as total_qty FROM `tabItem Batch Stock Balance` SB 
													INNER JOIN `tabBatch` B ON SB.batch_no = B.name
													WHERE SB.item_code = %(item_code)s AND B.expiry_date >= CURDATE() AND SB.warehouse=%(warehouse)s"""
													,{"item_code":x.item_code,"warehouse":spp_settings.source_warehouse},as_dict=1)
			# frappe.log_error(title="--t_qty_list--", message = t_qty_list)
			if t_qty_list:
				if t_qty_list[0].total_qty:
					if t_qty_list[0].total_qty>=x.consumed_qty:
						...
						batches_list = []
						# frappe.log_error(title = "-- batch wise stock --",message = batch_wise_stock)
						# on 28/8/23 if not qt the item not consumed in stock entry so flag checked.
						qty_check_flag = False
						error__data = {"material_reserved_info":[],"batch_qty_info":[]}
						# end
						for batch in batch_wise_stock:
							...
							# frappe.log_error(title = "-- res_qtys --",message = res_qtys)
							if res_qtys:
								if res_qtys[0].qty:
									error__data['material_reserved_info'].append({"batch_no":batch.batch_no,"material_reserved_qty":res_qtys[0].qty,"available_stock":batch.qty,"item":x.item_code})
									batch.qty = batch.qty - res_qtys[0].qty
							# frappe.log_error(title = "-- batch.qty --",message = batch.qty)
							if batch.qty>0:
								...
								# frappe.log_error(title = "-- s_qty --",message = s_qty)
								if round(s_qty,3)>0:
									# on 28/8/23 if not qt the item not consumed in stock entry so flag checked.
									qty_check_flag = True
									# end
									# frappe.log_error(title = "entering-- s_qty --",message = s_qty)
									...
									if not remaining_qty > 0:
										break
						# on 28/8/23 if not qt the item not consumed in stock entry so flag checked.
						if not qty_check_flag:
							if error__data.get('material_reserved_info'):
								html = f"The Stock is not available for the item <b>{x.item_code}</b> after <b>Material Reserved Stock</b> Deduction. Required Qty - <b>{x.consumed_qty}</b><br>"
								for idx,k in enumerate(error__data.get('material_reserved_info')):
									html += f"<b>{idx + 1}.</b>Batch - <b>{k.get('batch_no')}</b> , Ava Qty - <b>{k.get('available_stock')}</b> , Reserved Qty - <b>{k.get('material_reserved_qty')}</b><br>"
								rollback_transaction(batch_entry_id)
								frappe.throw(html)			
						# end
					else:
		...
		# check the all raw materials are consumed 28/8/23
		st_items = []
		wt_items = []
		for kk in stock_entry.items:
			if kk.item_code not in st_items:
				st_items.append(kk.item_code)
		for kb in work_order.required_items:
			wt_items.append(kb.item_code)
		if len(st_items) != len(wt_items):
			not_consumed_materials = []
			for kb in work_order.required_items:
				if kb.item_code not in st_items:
					not_consumed_materials.append(kb.item_code)
			if not_consumed_materials:
				html = f"The Following Items were not consumed for the manufaturing.<br>"
				for idx,k in enumerate(not_consumed_materials):
					html += f"<b>{idx + 1}.{k}</b>.<br>"
				rollback_transaction(batch_entry_id)
				frappe.throw(html)
			else:
				rollback_transaction(batch_entry_id)
				frappe.throw(f"Not all the <b>Raw Materials</b> are consumed in <b>Manufacturing Entry</b>.")
		#end 
		...
		spp_bno = s_nos[1].replace(s_nos[1][:3],"")
		stock_entry.append("items",{
			...
			"source_ref_document":"Batch ERP Entry",
			"source_ref_id":batch_entry_id
			})
		if spp_settings.auto_submit_stock_entries:
				stock_entry.docstatus=1
		
		stock_entry.insert(ignore_permissions=True)
		# frappe.log_error(title = f"-- stock entry {stock_entry.name}",message = stock_entry.as_dict())
		""" Reference """
		store__val = ""
		exe_entries = frappe.db.get_value("Batch ERP Entry",batch_entry_id,'stock_entry_reference')
		if exe_entries:
			exe_entries += "," + stock_entry.name
			store__val += exe_entries
		else:
			store__val = stock_entry.name
		frappe.db.set_value("Batch ERP Entry",batch_entry_id,'stock_entry_reference',store__val)
		frappe.db.commit()
		""" End """
		...
		frappe.log_error(message=frappe.get_traceback(),title=f"Stock Entry Data import failed - {batch_entry_id}")
		rollback_transaction(batch_entry_id)
...
def rollback_transaction(batch_entry_id):
	...
	frappe.db.commit()
	del_stock_wo_jc(batch_entry_id)

def del_stock_wo_jc(batch_entry_id):
	try:
		exe_entries = frappe.db.get_value("Batch ERP Entry",batch_entry_id,'stock_entry_reference')
		if exe_entries:
			all_st_entries = exe_entries.split(',')
			for st in all_st_entries:
				if frappe.db.get_value("Stock Entry",{"name":st}):
					exe_info = frappe.get_doc("Stock Entry",st)
					frappe.db.sql(f" DELETE FROM `tabStock Ledger Entry` WHERE voucher_type = 'Stock Entry' AND voucher_no = '{exe_info.name}' ")
					frappe.db.sql(f""" DELETE FROM `tabWork Order` WHERE name = '{exe_info.work_order}' """)
					frappe.db.sql(f""" DELETE FROM `tabJob Card` WHERE work_order = '{exe_info.work_order}' """)
					frappe.db.sql(f""" DELETE FROM `tabStock Entry` WHERE name = '{exe_info.name}' """)
					frappe.db.sql(f""" DELETE FROM `tabMaterial Reserved Stock` WHERE stock_entry_reference = '{exe_info.name}' """)
			frappe.db.commit()
	except Exception:
		frappe.log_error(title = 'rollback stock entries error', message = frappe.get_traceback())
		frappe.db.rollback()
		frappe.throw('Rollback <b>Stock Entries</b> error..!')
