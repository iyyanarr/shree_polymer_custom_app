...
# if job_card == "Completed":
# 	return {"status":"Failed","message":"The <b>Moulding Operation</b> for the scanned lot was completed..!"}
# check_lot_issue = frappe.db.sql(""" SELECT BI.bin,IB.compound,IB.spp_batch_number,BI.name,B.job_card,JB.name as job_card 
# 				FROM `tabBlank Bin Issue Item` BI 
# 				INNER JOIN `tabJob Card` JB ON JB.name= BI.job_card
# 				INNER JOIN `tabBlank Bin Issue` B ON B.name = BI.parent
# 				INNER JOIN `tabItem Bin Mapping` IB ON BI.bin = IB.blanking__bin
...
# if not check_lot_issue:
# 	return {"status":"Failed","message":"There is no entry for Blank Bin Issue <b>Or</b> the bin is already released for the scanned lot number."}
# if not check_lot_issue[0].compound:
# 	return {"status":"Failed","message":f"There is no <b>Compound</b> found in bin <b>{check_lot_issue[0].bin}</b>."}
# else:
...
# 		if job_card == "Completed":
# 			return {"status":"Failed","message":"The <b>Moulding Operation</b> for the scanned lot was completed..!"}
# 		check_lot_issue = frappe.db.sql(""" SELECT BI.bin,BI.name,B.job_card,JB.name as job_card 
# 						FROM `tabBlank Bin Issue Item` BI 
# 						INNER JOIN `tabJob Card` JB ON JB.name= BI.job_card
# 						INNER JOIN `tabBlank Bin Issue` B ON B.name = BI.parent
# 						WHERE BI.is_completed = 0 AND JB.batch_code=%(lot_no)s ORDER BY B.creation DESC """,{"lot_no":batch_no},as_dict=1)
# 		if not check_lot_issue:
# 			return {"status":"Failed","message":"There is no entry for Blank Bin Issue for the scanned lot number."}
# 		else:
# 			check_bin_release = frappe.db.sql(f" SELECT compound,spp_batch_number,qty FROM `tabItem Bin Mapping` WHERE blanking__bin = '{check_lot_issue[0].bin}' AND is_retired = 0 ",as_dict = 1)
# 			if not check_bin_release:
# 				return {"status":"Failed","message":f"The bin <b>{check_lot_issue[0].bin}</b> is already released..!"}
# 			else:
# 				if not check_bin_release[0].compound:
# 					return {"status":"Failed","message":f"There is no <b>Compound</b> found in bin <b>{check_lot_issue[0].bin}</b>."}
# 				else:
# 					check_lot_issue[0].update(check_bin_release[0])
# 			check_line_inspe_entry = frappe.db.get_value("Inspection Entry",{"lot_no":batch_no,"docstatus":1,"inspection_type":"Line Inspection"})
# 			if check_line_inspe_entry:
# 				# frappe.log_error(title = "lot info ", message = check_lot_issue[0])
# 				""" For fetching batch number from stock entry """
...
# 				""" For delivery note the batch number fetch from dc item table """
# 				batch_no = frappe.db.sql(f""" SELECT batch_no,docstatus,parenttype FROM `tabDelivery Note Item` WHERE spp_batch_no = '{check_lot_issue[0].spp_batch_number}'  """,as_dict=1)
# 				""" For old blanking entries are save as stock entry for that need to check in stock entry  """
# 				# frappe.log_error(title = "delivery note batch no ", message = batch_no)
# 				if not batch_no:
# 					spp_settings = frappe.get_single("SPP Settings")
# 					if not spp_settings.unit_2_warehouse:
# 						return {"status":"Failed","message":f"The default <b>Unit - 1 Warehouse</b> not found in <b>SPP Settings</b>..!"}	
...
# 					batch_no = frappe.db.sql(f""" SELECT SED.batch_no,SED.parenttype,SED.docstatus FROM `tabStock Entry` SE 
# 												  INNER JOIN `tabStock Entry Detail` SED ON SED.parent=SE.name WHERE SE.stock_entry_type="Material Transfer" 
# 												  AND SED.spp_batch_number = '{check_lot_issue[0].spp_batch_number}' AND SED.s_warehouse = '{spp_settings.default_sheeting_warehouse}' AND SED.t_warehouse = '{spp_settings.unit_2_warehouse}'  """,as_dict=1)
# 					if not batch_no:
# 						batch_no = frappe.db.sql(f""" SELECT SED.batch_no,SED.parenttype,SED.docstatus FROM `tabStock Entry` SE 
# 												  INNER JOIN `tabStock Entry Detail` SED ON SED.parent=SE.name WHERE SE.stock_entry_type="Repack" 
# 												  AND SED.spp_batch_number = '{check_lot_issue[0].spp_batch_number}' AND SED.t_warehouse = '{spp_settings.default_sheeting_warehouse}'  """,as_dict=1)
# 				if batch_no:
# 					if batch_no[0].docstatus == 1:
...
# 				if f__name:
# 					check_lot_issue[0].item_to_produce = f__name[0].item
# 					bom__ = frappe.db.sql(""" SELECT B.name,BI.item_code FROM `tabBOM Item` BI INNER JOIN `tabBOM` B ON BI.parent = B.name WHERE B.item=%(bom_item)s AND B.is_Active=1 """,{"bom_item":f__name[0].item},as_dict=1)
# 					boms__item = []
# 					for bm in bom__:
# 						boms__item.append(bm.item_code)
# 					if not check_lot_issue[0].compound in boms__item:
# 						return {"status":"Failed","message":f"There is no active BOM found for the bin <b>Compound - {check_lot_issue[0].compound}</b>"}
# 				else:
# 					return {"status":"Failed","message":f"BOM is not found for <b>Item to Produce</b>"}
...
# 	try:
# 		production__item =  frappe.db.get_value("Work Order",frappe.db.get_value("Job Card",self.job_card,"work_order"),"production_item")
# 		batch__rep,batch__no = generate_batch_no(batch_id = "T"+self.scan_lot_number,item = production__item,qty = flt(self.weight,3))
# 		if batch__rep:
# 			jc = frappe.get_doc("Job Card",self.job_card)
# 			for time_log in jc.time_logs:
...
# 			if work_order.operations:
# 				for operation in work_order.operations:
# 					frappe.db.set_value("Work Order Operation",operation.name,"completed_qty",flt(self.weight,3))
# 					frappe.db.commit()
# 			stock_entry = frappe.new_doc("Stock Entry")
# 			stock_entry.purpose = "Manufacture"
# 			stock_entry.work_order = work_order_id
...
# 			stock_entry.use_multi_level_bom = work_order.use_multi_level_bom
# 			stock_entry.stock_entry_type = "Manufacture"
# 			stock_entry.fg_completed_qty = flt(self.weight,3)
# 			if work_order.bom_no:
# 				stock_entry.inspection_required = frappe.db.get_value(
# 					"BOM", work_order.bom_no, "inspection_required"
# 				)
...
# 					"conversion_factor_uom":1,
# 					"is_finished_item":0,
# 					"transfer_qty":flt(self.weight,3),
# 					"qty":flt(self.weight,3),
...
# 						last_mov = frappe.db.sql(f" SELECT AMI.target_location FROM `tabAsset Movement` AM INNER JOIN `tabAsset Movement Item` AMI ON AM.name = AMI.parent WHERE AMI.asset = '{x.bin}' ORDER BY AMI.creation DESC LIMIT 1 ",as_dict = 1)
# 						if last_mov:
# 							if not last_mov[0].target_location == spp_settings.from_location:
# 								make_asset_movement(spp_settings,x)
# 						else:
# 							make_asset_movement(spp_settings,x)
# 						""" End """
# 					else:
...
# 						qty_info = frappe.db.sql(query, as_dict = 1)
# 						if qty_info:
# 							self.weight += flt(qty_info[0].qty,3)
# 							resp_ = make_stock_entry(self)
# 							if resp_.get('status') == 'failed':
# 								rollback_entries(self,resp_.get('message'))
# 							else:
...
# 		compound_available_qty = 0.0
# 		weight = flt(self.weight_without_shell,3)
# 		# batch_details = group_batch_qtys(self)
# 		inital_validate = True
# 		for is__bc in batch_details:
# 			if is__bc.get('is_balance_bin'):
# 				compound_available_qty += is__bb.get('consumed__qty')
# 			else:
# 				compound_available_qty += is__bb.get('qty')
# 		for is__bb in batch_details:
# 			if is__bb.get('is_balance_bin'):
# 				is__bb['consumed__qty_while_balance_bin'] = is__bb.get('consumed__qty')
# 				is__bb['balance__qty_while_balance_bin'] = is__bb.get('balance__qty')
# 				is__bb_compound_qty = flt(flt(is__bb.get('consumed__qty') / compound_available_qty,3) * flt(weight,3),3)
# 				is__bb['consumed__qty'] = is__bb_compound_qty
# 				consumed_qty = flt(consumed_qty + is__bb_compound_qty,3)
# 				is__bb["balance__qty"] = flt((flt(is__bb["qty"],3) - is__bb_compound_qty),3) 
# 		if consumed_qty != weight:
# 			for k in batch_details:
# 				compound__qty = flt((flt(k["qty"],3) / compound_available_qty) * flt(weight,3),3)
# 				if not k.get('is_balance_bin'): 
# 					if inital_validate and weight <= compound__qty:
# 						consumed_qty = weight
# 						k["consumed__qty"] = weight 
...
# 							k["is__consumed"] = 1
# 							break
# 		vcwspps = validate_consumption_with_spp_settings(self,batch_details)
# 		if vcwspps.get('status') == "success":
# 			# self.batch_details = json.dumps(batch_details)
# 			self.updated_batch_details = json.dumps(batch_details)
# 			return {"status":'success'}
# 		else:
# 			return vcwspps
# 	except Exception:
...
# 			if exe_b.get('batch_no__') == b__.get('batch_no__') and exe_b.get('spp_batch_number') == b__.get('spp_batch_number'):
# 				match_found = True
# 				exe_b['qty'] = flt((exe_b['qty'] + b__['qty']),3)
# 		if not match_found:
# 			final_batch_details.append(b__)
# 	return final_batch_details