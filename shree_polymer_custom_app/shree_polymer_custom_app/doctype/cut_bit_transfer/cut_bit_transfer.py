...
def validate_clip_barcode(batch_no,t_type,warehouse):
	...
	# ORIGINAL query - Keep the OR condition as it worked before
	stock_details = frappe.db.sql(""" SELECT S.name, SD.item_code,SD.item_name,SD.transfer_qty,SD.spp_batch_number,SD.batch_no,SD.stock_uom 
					  ...
					  LIMIT 1 """,{'mix_barcode':batch_no,'t_warehouse':s_warehouse},as_dict=1)
	
	if stock_details:
		items = stock_details[0].item_code
		
		 # FIX: Add ORDER BY to ensure consistent batch selection after indexing
		# Join Stock Entry to get creation date for proper ordering
		s_query = """SELECT I.item_code,I.item_name,I.description,I.batch_no,SD.spp_batch_number,SD.mix_barcode,
					I.stock_uom as uom,I.qty 
					...
					INNER JOIN `tabStock Entry` S ON SD.parent = S.name
					WHERE I.item_code = '{item_codes}' 
					  AND (SD.mix_barcode = '{mix_barcode}' OR SD.barcode_text = '{mix_barcode}')
					  AND I.qty > 0 
					  AND I.warehouse = '{warehouse}' 
					  AND B.expiry_date >= CURDATE()
					ORDER BY S.creation DESC, B.creation DESC
					LIMIT 1""".format(
						cutbit_warehouse=spp_settings.default_cut_bit_warehouse,
		...
		if not st_details:
			s_query = """SELECT I.item_code,I.item_name,I.description,I.batch_no,SD.spp_batch_number,SD.mix_barcode,
					...
					LIMIT 1""".format(
						...
						mix_barcode=batch_no,
						item_codes=items,
						warehouse=spp_settings.unit_2_warehouse
					)
			st_details = frappe.db.sql(s_query,as_dict=1)
