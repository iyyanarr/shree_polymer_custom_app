...
frappe.ui.form.on('Add On Work Planning', {
	...
	"scan_supervisor": (frm) => {
		if (frm.doc.scan_supervisor && frm.doc.scan_supervisor != undefined) {
			 frappe.call({
				method: 'shree_polymer_custom_app.shree_polymer_custom_app.doctype.add_on_work_planning.add_on_work_planning.validate_supervisor',
				args: {
	...
	set_bom_val_in_child(frm){
		if(frm.bom__values){
			...
			frappe.call({
				method: 'shree_polymer_custom_app.shree_polymer_custom_app.doctype.add_on_work_planning.add_on_work_planning.get_validate_bom',
				args: {
					self:frm.doc
				},
				freeze: true,
...
frappe.ui.form.on("Add On Work Plan Item",{
	bom(frm,cdt,cdn){
