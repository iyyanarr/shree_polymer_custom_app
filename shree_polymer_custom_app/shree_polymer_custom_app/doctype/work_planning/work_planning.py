...
from frappe.utils import cint, cstr, duration_to_seconds, flt, update_progress_bar,format_time, formatdate, getdate, nowdate,now,add_to_date
		...
		try:
			...
			for item in doc_info.items:
				...
				wo.append("operations",{
					...
					# "planned_end_time":add_to_date(now(), minutes=int(spp_settings.default_time))
					})
...
def update_job_cards(wo,actual_weight,doc_info,item,production_mat_item):
	try:
		...
		for job_card in job_cards:
			...
			press_info = frappe.db.get_all("Press Mould Specification",filters={"mould":resolved_mould,"press":item.get('work_station')},fields=["*"])
			if press_info:
...
@frappe.whitelist()
def submit_workplan(docname,doctype,doc_info):
	try:
		import json
		...
		if doc_info:
			for k in json.loads(doc_info):
				frappe.db.set_value("Work Plan Item",k.get('name'),"bom",k.get('bom'))
			frappe.db.commit()
		if res:
...
def get_mould_list(mat):
	try:
		mould_ref_list = frappe.db.get_all("Mould Specification",{"spp_ref":mat,"mould_status":["in",["ACTIVE","SPARE","DEV"]],"docstatus":1},["mould_ref"])
		if mould_ref_list:
			frappe.response.status = "success"
