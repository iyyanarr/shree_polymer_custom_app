...
# class LineInspectionEntry(Document):
# 	def validate(self):
# 		if not self.items:
...
# @frappe.whitelist()
# def validate_lot_barcode(bar_code,docname):
# 	try:
# 		exe_prodt_lot = frappe.db.get_all("Line Inspection Entry",filters={"name":("!=",docname),"docstatus":1,"scan_production_lot":bar_code})
# 		if not exe_prodt_lot:
# 			production_lot = frappe.db.sql(f""" SELECT name FROM `tabJob Card` WHERE batch_code='{bar_code}'  """,as_dict=1)
# 			if production_lot:
# 				employee = frappe.db.sql(""" SELECT JB.name as job_card,JB.production_item as product_ref_no,JB.batch_code,
# 											E.employee_name as operator_name FROM `tabJob Card` JB 
# 											LEFT JOIN `tabJob Card Time Log` LG ON LG.parent = JB.name
# 											LEFT JOIN `tabEmployee` E ON LG.employee = E.name
# 											WHERE JB.batch_code=%(lot_no)s """,{"lot_no":bar_code},as_dict=1)
# 				frappe.response.status = 'success'
# 				frappe.response.message = employee[0]
# 			else:
# 				frappe.response.status = 'failed'
# 				frappe.response.message = "Scanned lot <b>"+bar_code+"</b> not exist."
# 		else:
# 			frappe.response.status = 'failed'
# 			frappe.response.message = f"Scanned lot <b>${bar_code}</b> already inspected."
# 	except Exception:
# 		frappe.log_error(message=frappe.get_traceback(),title="shree_polymer_custom_app.shree_polymer_custom_app.doctype.line_inspection_entry.line_inspection_entry.validate_lot_barcode")
# 		frappe.response.status = 'failed'
...
# 		frappe.log_error(message=frappe.get_traceback(),title="shree_polymer_custom_app.shree_polymer_custom_app.doctype.line_inspection_entry.line_inspection_entry.make_stock_entry")
# 		frappe.db.rollback()
# Copyright (c) 2023, Tridotstech and contributors
# For license information, please see license.txt
# Copyright (c) 2023, Tridotstech and contributors
...
class LineInspectionEntry(Document):
	def validate(self):
		if self.lot_no:
			check_exist = frappe.db.get_all("Line Inspection Entry",filters={"name":("!=",self.name),"docstatus":1,"lot_no":self.lot_no})
			if check_exist:
				frappe.throw(f"Line Inspection Entry for lot <b>{self.lot_no}</b> already exists..!")
		if self.items:
			for each_item in self.items:
				...
				# query = f"""  SELECT name FROM `tabLine Inspection Entry Item` WHERE lot_no='{each_item.lot_no}' AND parent != '{self.name}' AND docstatus=1 """
				# exe_rec = frappe.db.sql(query,as_dict=1)
				# if exe_rec:
				# 	if each_item.lot_no == self.lot_no:
				# 		frappe.throw(f"Line Inspection Entry for lot <b>{self.lot_no}</b> already exists..!")
				# 	else:
				# 		frappe.throw(f"Line Inspection Entry for lot <b>{each_item.lot_no}</b> in row <b>{each_item.idx}</b> is already exists..!")
...
def validate_lot_number(batch_no,docname):
	try:
		check_exist = frappe.db.get_all("Line Inspection Entry",filters={"name":("!=",docname),"docstatus":1,"lot_no":batch_no})
		if check_exist:
			return {"status":"Failed","message":"Already Line Inspection Entry is created for this lot number."}
		...
		frappe.log_error(message=frappe.get_traceback(),title="shree_polymer_custom_app.shree_polymer_custom_app.doctype.line_inspection_entry.line_inspection_entry.validate_lot_number")
		...
		if bom:
			...
			if check_uom:
				...
				if flt(t_qty, 3)>0:
					...
					naming_status,naming_series = get_stock_entry_naming_series(spp_settings,"Line Inspection")
					if naming_status:
	...
	try:
		...
		if spp_settings and spp_settings.designation_mapping:
			for desc in spp_settings.designation_mapping:
				if desc.spp_process == "Line Inspector":
					if desc.designation:
		...
		frappe.log_error(message=frappe.get_traceback(),title="shree_polymer_custom_app.shree_polymer_custom_app.doctype.line_inspection_entry.line_inspection_entry.validate_inspector_barcode")
		frappe.response.status = 'failed'
		frappe.response.message = "Something went wrong."