...
class StockBalanceReport(object):
	...
	def run(self):
		...
		# frappe.log_error(title="--------sle_entries",message=self.sle_entries[0])
		self.prepare_new_data()
		# frappe.log_error(title="--------prepare_new_data",message=self.data[0])
	...
	def get_item_warehouse_map(self):
		...
		for entry in self.sle_entries:
			# frappe.log_error(title='--ledger data--', message = entry)
			group_by_key = self.get_group_by_key(entry)
	...
	def prepare_item_warehouse_map(self, item_warehouse_map, entry, group_by_key):
		...
		# frappe.log_error(title="-- qty diff --", message = qty_diff)
		qty_dict.val_rate = entry.valuation_rate
		qty_dict.bal_qty += qty_diff
		qty_dict.bal_val += value_diff
		# frappe.log_error(title="-- after qty dict --", message = qty_dict)
		...
		item_warehouse_map[group_by_key] = frappe._dict(
			{
				...
				"spp_batch_no":entry.sed_spp_batch_no if entry.sed_spp_batch_no else entry.dni_spp_batch_no,
				"voucher_type":entry.sed_voucher_type if entry.sed_voucher_type else entry.dn_voucher_type,
				"voucher_no":entry.sed_voucher_no if entry.sed_voucher_no else entry.dn_voucher_no,
				# end
			}
		)
	...
	def get_group_by_key(self, row) -> tuple:
		...
		group_by_key = [row.company, row.item_code, row.warehouse, row.batch_no]
		# end
	...
	def prepare_stock_ledger_entries(self):
		...
		delivery_note_detail = frappe.qb.DocType("Delivery Note Item")
		delivery_note_ = frappe.qb.DocType("Delivery Note")
		query = (query.left_join(stock_entry_detail)
						.on((stock_entry_detail.parent == sle.voucher_no)
							& (sle.voucher_type == 'Stock Entry')
							& (stock_entry_detail.batch_no == sle.batch_no)
							& (stock_entry_detail.docstatus == 1)
							& (stock_entry_detail.item_code == sle.item_code)
							)
						.select(stock_entry_detail.spp_batch_number.as_('sed_spp_batch_no'),
								stock_entry_detail.source_ref_document.as_('sed_voucher_type'),
								stock_entry_detail.source_ref_id.as_('sed_voucher_no'),
								stock_entry_detail.parent.as_('sle_voucher__')
								).distinct()
								# )	
					)
		query = (query.left_join(delivery_note_detail)
						.on((delivery_note_detail.parent == sle.voucher_no) 
							& (sle.voucher_type == 'Delivery Note')
							& (delivery_note_detail.batch_no == sle.batch_no)
							& (delivery_note_detail.docstatus == 1)
							& (delivery_note_detail.item_code == sle.item_code)
							)
						.select(delivery_note_detail.spp_batch_no.as_('dni_spp_batch_no'),
								delivery_note_detail.parent.as_('dni_voucher__')
								).distinct()
						)
		query = (query.left_join(delivery_note_)
						.on((delivery_note_.name == sle.voucher_no) 
							& (sle.voucher_type == 'Delivery Note')
							& (delivery_note_.docstatus == 1)
							)
						.select(delivery_note_.reference_name.as_('dn_voucher_no'),
	      						delivery_note_.reference_document.as_('dn_voucher_type'),
	      						delivery_note_.name.as_('dn_voucher__')
								).distinct()
						)
		if self.filters.get('spp_batch_no'):
			query = query.where((stock_entry_detail.spp_batch_number == self.filters.get('spp_batch_no')) | (delivery_note_detail.spp_batch_no == self.filters.get('spp_batch_no')))
		...
		# frappe.log_error("--self.sle_entries--",self.sle_entries)
	...
	def get_columns(self):
		...
		columns.extend(
			[
				...
				{"label": _("Source Document Type"), "fieldname": "voucher_type", "width": 200},
				{
...
#customize
def filter_items_with_no_transactions(iwb_map, float_precision: float, inventory_dimensions: list = None):
	pop_keys = []
	for group_by_key in iwb_map:
		qty_dict = iwb_map[group_by_key]
		no_transactions = True
		no_balance_qty = False
		for key, val in qty_dict.items():
			...
			if key in [
				...
				continue
			com_key = ["posting_date","date","voucher_no","voucher_type","batch_no","spp_batch_no"]
			if key in com_key:
				continue
...
# query = frappe.qb.from_(
		# 	(query)
		# ).select('item_code','warehouse','posting_date','actual_qty','valuation_rate',
		# 			'company','voucher_type','qty_after_transaction','stock_value_difference',
		# 			'name','voucher_no','stock_value','batch_no','serial_no','item_group',
		# 			'stock_uom','item_name','spp_batch_no',)
...
# stock_entry_detail = frappe.qb.DocType("Stock Entry Detail")
		...
		# 				.on((stock_entry_detail.parent == sle.voucher_no)
		# 					& (sle.voucher_type == 'Stock Entry')
		# 					& (sle.batch_no == stock_entry_detail.batch_no))
		# 				.select(stock_entry_detail.spp_batch_number.as_('spp_batch_no'),
		# 						stock_entry_detail.source_ref_document.as_('voucher_type'),
		# 						stock_entry_detail.source_ref_id.as_('voucher_no'))
		# 			).where((stock_entry_detail.docstatus == 1)
		# 					& (stock_entry_detail.item_code == sle.item_code))
		# query = (query.left_join(delivery_note_detail)
		# 				.on((delivery_note_detail.parent == sle.voucher_no) 
		# 					& (sle.voucher_type == 'Delivery Note')
		# 					& (sle.batch_no == delivery_note_detail.batch_no))
		# 				.select(delivery_note_detail.spp_batch_no.as_('spp_batch_no'),
		# 						)
		# 				).where((delivery_note_detail.docstatus == 1)
		# 						& (delivery_note_detail.item_code == sle.item_code))
		# if self.filters.get('spp_batch_no'):
		# 	query = query.where((stock_entry_detail.spp_batch_number == self.filters.get('spp_batch_no')))
		# 	query = query.where((delivery_note_detail.spp_batch_no == self.filters.get('spp_batch_no')))
		# if self.filters.get('batch_no'):
		# 	query = query.where((sle.batch_no == self.filters.get('batch_no')))